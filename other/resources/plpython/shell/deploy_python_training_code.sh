if [ -z "$OVERRIDE_VENDORED_DEPS" ] && [ ! -d "${SHARED_PATH}/mlapp" ]
then echo "Skipping reinstall of vendored dependencies - OVERRIDE_VENDORED_DEPS is empty";
else ssh -i ${SCP_PEM_PATH} ${USER}@${HOST} -o "StrictHostKeyChecking=no" "rm -rf ${SHARED_PATH}/mlapp ${SHARED_PATH}/mlappbase ${SHARED_PATH}/config; \
