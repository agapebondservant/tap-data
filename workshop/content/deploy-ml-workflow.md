...
```execute
export DATA_E2E_GIT_TOKEN={{DATA_E2E_GIT_TOKEN}} && export DATA_E2E_GIT_USER={{DATA_E2E_GIT_USER}} && git clone https://${DATA_E2E_GIT_USER}:${DATA_E2E_GIT_TOKEN}@github.com/${DATA_E2E_GIT_USER}/sample-ml-app.git ~/sample-ml-app
```
...
**Image Resources** know how to transform source code into images by using both declarative and imperative image building tools.
...
```execute
REGISTRY_PASSWORD={{DATA_E2E_REGISTRY_PASSWORD}} ~/other/resources/bin/kp secret create tbs-demo-docker-secret --dockerhub {{DATA_E2E_REGISTRY_USERNAME}}
```
...
Let's view the logs as our image is generated:
```execute
kp build logs imgprocessor --namespace {{session_namespace}}
```
<font color="red">NOTE:</font> After a few seconds, you may click **Ctrl-C** to exit the logs.
