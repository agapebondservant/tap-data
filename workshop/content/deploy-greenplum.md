...
```execute
clear  && mc config host add --insecure data-e2e-minio https://{{DATA_E2E_MINIO_PLAIN_URL}} {{DATA_E2E_MINIO_PLAIN_ACCESS_KEY}} {{DATA_E2E_MINIO_PLAIN_SECRET_KEY}} && kubectl create secret generic data-e2e-greenplum-pxf-configs --from-literal='access_key_id={{DATA_E2E_MINIO_PLAIN_ACCESS_KEY}}' --from-literal='secret_access_key={{DATA_E2E_MINIO_PLAIN_SECRET_KEY}}' --namespace=greenplum-system --dry-run -o yaml | kubectl apply -f - && mc mb -p --insecure data-e2e-minio/pxf-config && mc mb --insecure -p data-e2e-minio/pxf-data/{{ session_namespace }} && mc cp --insecure ~/other/resources/greenplum/minio-site.xml data-e2e-minio/pxf-config/servers/default/minio-site.xml && mc cp --insecure ~/other/resources/data/*.csv data-e2e-minio/pxf-data/{{ session_namespace }}
```
