...
helm install --set \
resources.requests.memory=1.5Gi,tls.enabled=true,auth.rootUser=minio,\
ingress.enabled=true,ingress.hostname="${DATA_E2E_MINIO_URL}",ingress.selfSigned=true,tls.autoGenerated=true \
--namespace minio minio oci://registry-1.docker.io/bitnamicharts/minio --wait
kubectl apply -f resources/minio-bitnami-http-proxy.yaml --namespace minio
export DATA_E2E_MINIO_ACCESS_KEY=$(kubectl get secret --namespace minio minio -o jsonpath="{.data.root-user}" | base64 -d)
export DATA_E2E_MINIO_SECRET_KEY=$(kubectl get secret --namespace minio minio -o jsonpath="{.data.root-password}" | base64 -d)
...
#########################################################################
echo "Installing Minio endpoint (without TLS)..."
kubectl create ns minio-plain || true
helm install --set \
resources.requests.memory=1.5Gi,auth.rootUser=minio,\
ingress.enabled=true,ingress.hostname="minio-plain.${DATA_E2E_BASE_URL}" \
--namespace minio-plain minio oci://registry-1.docker.io/bitnamicharts/minio --wait
kubectl apply -f resources/minio-bitnami-plain-http-proxy.yaml
export DATA_E2E_MINIO_PLAIN_ACCESS_KEY=$(kubectl get secret --namespace minio-plain minio -o jsonpath="{.data.root-user}" | base64 -d)
export DATA_E2E_MINIO_PLAIN_SECRET_KEY=$(kubectl get secret --namespace minio-plain minio -o jsonpath="{.data.root-password}" | base64 -d)
...
echo "Minio endpoint without TLS installed."
